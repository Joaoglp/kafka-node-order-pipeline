...
describe('analyzeOrder', () => {
    ...
    it('throws an error when claude returns invalid JSON', async () => {
        ...
        await expect(analyzeOrder(sampleOrder)).rejects.toThrow('Failed to parse Claude response as JSON');
    });
    ...
    it('throws an error when claude returns no text block at all', async () => {
        ...
        await expect(analyzeOrder(sampleOrder)).rejects.toThrow('No text response from Claude');
    });
});